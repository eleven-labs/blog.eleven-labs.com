...
<p>Ici, on demande donc la création d'un <em>echange </em>nommé "default", et d'une <em>queue </em>"send_astronaut_to_space", associé à notre échange par une <em>routing key </em>homonyme.<br />
Un <em>binding </em>est une relation entre un <em>exchange </em>et une <em>queue.</em></p>
<p>Lançons la commande pour la création de notre vhost :</p>
...
<p>En vous connectant sur votre interface RabbitMQ management (ex: http://127.0.0.1:15672/), plusieurs choses apparaissent :</p>
...
<p>En cliquant sur l'onglet <em>Exchanges</em>, un exchange <em>default</em> a été créé avec un <em>binding </em>avec notre <em>queue</em>, comme indiqué dans la console.</p>
...
<p>Voici donc un exemple de configuration. La partie intéressante arrive à partir du paramètre <em>consumers</em>.</p>
<p>Chaque message publié dans un <em>exchange</em> sera acheminé vers une <em>queue</em> en fonction de sa <em>routing key</em>. Ainsi, il nous est donc nécessaire de traiter une message stocké dans une <em>queue</em>. Dans Swarrot, ce sont les <em>processors</em> qui s'occcupent de cela.<br />
Pour consommer notre message, il nous est donc nécessaire de créer notre propre <em>processor</em>. Comme indiqué dans la documentation, un <em>processor</em> est simplement un service Symfony qui doit implémenter l'interface <em>ProcessorInterface</em>.</p>
...
<p>Le secret est de déclarer un nouveau type de message, en spécificant la <em>connection</em>, l'<em>exchange</em>, et la <em>routing key</em>, et de publier un message de cette façon :</p>
...
<p>Avec Swarrot, la gestion des <em>retries</em> est très facile à mettre en place. Vous vous souvenez des <em>middlewares </em>dont je vous ai parlé plus haut ? Et bien, il existe un <em>middleware</em> pour ça !</p>
<pre class="theme:sublime-text lang:yaml decode:true"># app/config/config.yml
    consumers:
# ...
            middleware_stack:
...
Si l'on regarde les détails de la queue <em>send_astronaut_to_space</em>, on voit que "<em>x-dead-letter-exchange</em>" a pour valeur "<em>dl</em>", et que "<em>x-dead-letter-routing-key</em>" a pour valeur "<em>send_astronaut_to_space</em>", correspondant à notre <em>binding </em>précédemment expliqué.</p>
