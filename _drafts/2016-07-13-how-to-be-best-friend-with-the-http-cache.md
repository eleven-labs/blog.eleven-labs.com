...
I am currently lead developer for a French press website with very high traffic ([lemonde.fr](http://www.lemonde.fr/)). Along my previous work experiences, I was able to perform development on several other high volumetry sites as well.
...
|------------------------------------------------------------------------------------------|----------------------------------------------|
| [Firefox](https://fr.wikipedia.org/wiki/Firefox "Firefox")                               | Ctrl + F5                                    |
| [Chrome](https://fr.wikipedia.org/wiki/Chrome "Chrome")                                  | Ctrl + F5 ou Shift + F5 ou Ctrl + Shift + N  |
| [Safari](https://fr.wikipedia.org/wiki/Safari_(logiciel) "Safari (logiciel)")            | Ctrl + Alt + E                               |
...
As explained in the first chapter, the key of the HTTP cache is the URL of the page. This can be a nuisance if your page is dynamic depending on the user, because the URL will be the same, but the content will be different. It will therefore be necessary to cache several contents for one URL. Fortunately, [Tim Berners-Le](https://fr.wikipedia.org/wiki/Tim_Berners-Lee "Tim Berners-Lee"), the inventor of the HTTP protocol provided for this event by adding the header:
...
The Content Delivery Network is a network of computers to serve content (https://fr.wikipedia.org/wiki/Content\_delivery\_network). It is external to the architecture and its specialty is geolocation, it therefore allows a user to go onto the nearest CDN. You have probably already used one when you use a JS tag, e.g.: jquery, angular, etc. …, and if you use the URL provided rather than the downloaded file.
...
If it was not for the Grace, Varnish would have to call the server directly to obtain the response to be returned to the user. But with the Grace, Varnish will deliver its cache and ask the server for a new content, which allows you to avoid latency time for the user.
...
Edge Side Include (ESI) allows you to use the full power of Varnish. As stated above, this technology was invented by [Akamai](https://www.akamai.com/fr/fr/), one of the most famous CDNs.
...
For further information, I suggest you look at a previous article [ http://blog.eleven-labs.com/symfony-2-cache-http-esi/](http://blog.eleven-labs.com/symfony-2-cache-http-esi/), which explains an implementation for Symfony.

You can also find a presentation about HTTP caches and Symfony [here](https://docs.google.com/presentation/d/1RVr_JfpFKVRXdg4hy6war3OfiSJtdeYzFsdxhn2t0NY/edit?usp=sharing).
