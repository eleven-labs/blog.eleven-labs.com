...
**Correction** : suppression des trois canoniques locales (`useHomePageContainer`, `useCategoryPageContainer`, `useSearchPageContentContainer`). Le layout reste seul responsable, avec une canonique absolue et auto-référente, sauf pour `/fr/` qui pointe vers `/` : la racine sert la même page et c'est l'URL que Google a retenue pour l'accueil.
...
`getSitemapEntries` construit désormais une date de dernière modification (`updatedAt` à défaut `date`) pour chaque article, tutoriel et étape de tutoriel : 479 des 541 URL en sont pourvues, les autres étant l'accueil et les pages catégorie, dont la date de modification n'a pas de sens stable.

`priority` et `changefreq` sont retirés dans la foulée (voir 5.7), ainsi que l'espace de noms `xmlns:news` déclaré sans être utilisé : le sitemap passe de 93 Ko à 75 Ko sans perdre aucune information exploitée par Google.
...
- La racine `/` est aujourd'hui la page canonique retenue par Google pour l'accueil, et `/fr/` est classée « Page en double sans URL canonique sélectionnée par l'utilisateur ». La branche confirme ce choix plutôt que de l'inverser : la canonique de `/fr/`, le `loc` du sitemap, le `hreflang` `fr` et `x-default`, le JSON-LD `WebSite` et les liens internes vers l'accueil pointent tous vers `/`. Basculer vers `/fr/` aurait été plus cohérent avec les autres pages, toutes préfixées par la langue, mais aurait imposé plusieurs semaines d'instabilité sur l'accueil le temps que Google transfère les signaux, pour un gain nul.
...
- Aucun résultat enrichi de type `Article` n'a jamais été détecté, uniquement `Breadcrumbs` (issus de la microdonnée du design system, désormais remplacée par le JSON-LD, voir 3.5). Le logo invalide en était la cause probable.
...
### 3.5 Doublon `BreadcrumbList` — appliqué

Le composant `Breadcrumb` du design system émettait un `BreadcrumbList` en microdonnées, et la branche ajoutait un `BreadcrumbList` en JSON-LD sur les articles. Le problème n'était pas tant la coexistence des deux formats, que Google tolère, que leur **désaccord** : sur un article, la microdonnée décrivait `Accueil > Catégorie` et le JSON-LD `Accueil > Catégorie > Article`, voire `> Étape` sur un tutoriel. Deux fils d'Ariane différents pour une même page, Google devant choisir lequel afficher.

Depuis que le design system est intégré au dépôt, l'arbitrage n'a plus à être porté ailleurs :

- la microdonnée est retirée du composant `Breadcrumb`, qui redevient purement visuel ;
- le JSON-LD devient la source unique, via un hook `useBreadcrumbListSchema` partagé : les articles et tutoriels gardent leur fil complet, et les pages catégorie, qui n'avaient jusqu'ici que la microdonnée, reçoivent `Accueil > Catégorie` en JSON-LD ;
- les pages auteur n'ont pas de fil d'Ariane, et sont de toute façon en `noindex` (3.6).

### 3.6 Pages auteur à faible valeur — appliqué

Le sitemap compte une part notable de pages auteur et de pages auteur paginées, au contenu mince (une biographie et une liste), qui consomment du budget de crawl sans générer de clic.

Appliqué :

- `noindex, follow` sur toutes les pages auteur, paginées comprises (`useAuthorPageContainer`). Le `follow` conserve la découverte des articles depuis ces pages.
- Pages auteur retirées du sitemap : une URL en `noindex` n'a rien à y faire, Google considère le sitemap comme la liste des pages à indexer. Le sitemap passe de 674 à 541 URL.
- Les pages restent générées par le prerender, qui prenait jusqu'ici sa liste d'URL dans le sitemap : elles y sont désormais ajoutées explicitement, sans quoi elles auraient répondu en 404.

Le signal d'expertise décrit en 5.7 n'en pâtit pas : il repose sur la signature et la biographie en pied d'article et sur `author.url` dans le JSON-LD, qui restent en place. Une page en `noindex` reste explorable et peut toujours servir de référence d'auteur.
...
Les chantiers de contenu sont suivis dans le jalon [SEO](https://github.com/eleven-labs/blog.eleven-labs.com/milestone/3).

### 4.1 Priorité 1 — réécrire les titres et descriptions des pages en première page

**Ticket** : [#1252](https://github.com/eleven-labs/blog.eleven-labs.com/issues/1252)
...
**Ticket** : [#1253](https://github.com/eleven-labs/blog.eleven-labs.com/issues/1253)
...
**Ticket** : [#1254](https://github.com/eleven-labs/blog.eleven-labs.com/issues/1254)
...
**Ticket** : [#1255](https://github.com/eleven-labs/blog.eleven-labs.com/issues/1255)
...
La dette est concentrée sur l'ancien contenu : 294 images sans texte alternatif sur 941 entre 2015 et 2021, 9 sur 244 depuis 2022, aucune depuis 2025.

**Appliqué** : `pnpm validate-markdown` signale désormais chaque image sans texte alternatif (fichier et ligne), en ignorant les blocs de code. En CI, chaque image produit une annotation `::warning` : un auteur qui ajoute une image sans `alt` la voit directement sur la ligne concernée dans le diff de sa PR. C'est un avertissement et non une erreur, pour ne pas bloquer la CI sur l'existant. La documentation de rédaction demande aussi un nom de fichier descriptif. La reprise des 302 images existantes reste un travail éditorial, idéalement en commençant par les articles qui génèrent encore des impressions.
...
- **`priority` et `changefreq` du sitemap sont très largement ignorés** — le réglage fin (1.0 pour les articles, 0.9 pour les étapes, 0.8 pour l'accueil…) ne servait à rien. **Appliqué** : les deux balises sont retirées, seul `lastmod` (1.7) est conservé.
- **La balise `meta keywords` est inutilisée** — elle n'est pas présente dans le dépôt, rien à faire.
- **L'accumulation de mots-clés est contraire aux règles** — et Google comprend seul les variantes sémantiques. Côté code, rien à corriger : aucun mot-clé n'est injecté dans les pages. Le champ `keywords` du front matter (renseigné sur 401 articles) n'est d'ailleurs lu par aucun code — ni balise, ni JSON-LD, ni indexation Algolia — alors que la documentation de rédaction le présentait comme « used for SEO and search ». **Appliqué** : la documentation est corrigée pour ne plus laisser croire aux auteurs qu'il a un effet. Le point reste éditorial : ne pas répéter les mêmes termes dans le titre, la description et le corps.
...
Le reste des prérequis est en place : `robots.txt` n'exclut aucune image, les illustrations sont de vrais `<img>` présents dans le HTML rendu côté serveur, les couvertures sont déclarées dans `og:image` et le JSON-LD, et `max-image-preview:large` autorise les grandes vignettes. Un sitemap d'images n'apporterait presque rien : Google le recommande pour les images qu'il ne découvrirait pas autrement, ce qui n'est pas le cas ici.

## 6. À surveiller après mise en production

1. **Accueil** : vérifier dans le rapport d'indexation que `/` reste « Indexée » et que `/fr/` passe de « Page en double sans URL canonique sélectionnée par l'utilisateur » à « Page en double avec balise canonique correcte ».
...
7. **`hreflang` de l'accueil** : contrôler dans le rapport de ciblage international qu'aucune erreur de réciprocité n'apparaît entre `/`, `/en/` et le `x-default`.
8. **Désindexation des pages auteur** : vérifier qu'elles passent en « Exclue par la balise noindex » et que le nombre d'URL du sitemap baisse d'autant.
9. **Fils d'Ariane** : vérifier dans le rapport « Fils d'Ariane » que les éléments valides se maintiennent après le retrait de la microdonnée, et qu'ils apparaissent désormais aussi sur les pages catégorie.
