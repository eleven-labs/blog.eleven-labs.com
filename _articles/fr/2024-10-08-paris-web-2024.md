...
C'est donc à l'institut Pasteur que nous nous sommes rendus pendant deux jours. Dès la conférence d'ouverture nous avons pu nous rendre compte des moyens mis en oeuvre pour accueillir le plus de personnes possibles&nbsp;: les conférences étaient toutes interprétées en LSF (langues des signes française) et une retranscription en direct par vélotypie, à la vitesse de la parole, était affichée de chaque côté de la présentation centrale. Le lieu était évidemment accessible PMR (personnes à mobilité réduite). Les conférences se sont déroulées les unes à la suite des autres, pas besoin donc de faire de choix, nous avons pu toutes les suivre&nbsp;! Voici ce que nous en avons retenu.
...
En juxtaposant deux mots au sens contraire, ils se vident de leur sens. *Exemple&nbsp;?*

Pour l’orateurice (mot créé pour la conférence pour désigner les orateurs et les oratrices), le langage inclusif, c’est tout ce qu’on peut mobiliser au niveau de notre communication, orale ou écrite, pour éviter d’exclure, opprimer ou invisibiliser une partie de la population. Ce n'est donc pas uniquement le point médian ou l'écriture inclusive, qui aujourd'hui proposent des solutions mais peuvent avoir aussi des défauts comme complexifier la lecture.

Le français est une langue genrée, cela demande donc plus d'efforts par rapport à d'autres langues moins genrées pour mettre en place des changements liés au genre.
...
Magali se demande&nbsp;: pourquoi dans une industrie proactive vers le changement technique comme la tech, on n’arrive pas à être aussi proactif dans le progrès social&nbsp;?
...
## "Orejime, un projet open-source et accessible&nbsp;: compte de fée ou drama project&nbsp;?!" par Simon Bonaventure
...
Ils implémentent donc des améliorations pour rendre le gestionnaire de cookie accessible, mais se rendent compte que petit à petit leurs modifications sont remplacées par celles d'autres personnes qui ne tiennent pas compte de l'accessibilité. Ils décident donc de faire un fork du projet et lancent leur propre projet&nbsp;: Orejime.
...
- et d'autres...
2. Ils en tirent des bénéfices pour leur SEO car un lien vers leur site est présent sur chaque gestionnaire de cookie
...
Il existe de nouveaux outils très interessants&nbsp;: [HTMX](https://htmx.org/), [Hotwire Turbo](https://turbo.hotwired.dev/) et [Unpoly](https://unpoly.com/), les CustomElements, [Hotwire Stimulus](https://stimulus.hotwired.dev/).
...
-   Pourquoi ce n’est pas accessible&nbsp;: est-ce des problèmes avec les personnes&nbsp;? Le processus&nbsp;? Les outils&nbsp;?

Un des premier conseil, c'est d'être empathique avec les gens qui résistent pour trouver des arguments qui correspondent à leurs problèmes. Cela permet de trouver un passage au travers de la résistance au changement.
...
Le duo de Flora et Esther nous présentent une conférence à la fois théorique et pratique autour du design et du temps.
...
Elle nous parle aussi de Fabien Fenouillet qui effectue des études en psychologie cognitive sur la motivation, le choix, et lié à l'école, l'enseignement, les enfants.
...
Il faut arrêter de se focaliser uniquement sur le taux.

Le mieux est de se demander comment empêcher les erreurs d'accessibilité avant qu'elles ne se fassent et deviennent de plus en plus nombreuses.
...
On a une image des mails comme étant léger, s'envolant dans les airs pour arriver à leur destinataire. En réalité ils passent par un réseau bien physique de câbles sous-marins.

Les données présentées par Thomat sur les câbles sous-marins proviennent de [https://www2.telegeography.com/](https://www2.telegeography.com/). On peut utiliser le site [iana.org](https://www.iana.org/) pour connaître l’adresse du domaine mail (ou foyer numérique)
...
Les derniers ajouts dans les WCAG liés aux troubles cognitifs date de 2018.
...
- Proposer des présentation alternatives
- Eviter les bruits parasites qui peuvent perturber la concentration
- Limiter et faciliter la saisie d’information
...
- Utiliser des contrastes de couleur suffisant
...
Les typographies évoluent avec le temps, de l'imprimante et la machine à écrire, à nos jours. Dans les années 90, une technologie appelée MultipleMaster arrive. C’est l’ancêtre des polices variables mais à cause de sa complexité, cette technologie ne parvient pas à se faire une place dans les usages.
...
-   Une mauvaise compréhension du comportement attendu, ce qui fait qu'on test des cas qui ne se produisent pas ou se produisent différemment
...
La première étape est de quantifier les problèmes, former et informer des erreurs rencontrées.
...
2.  Qui je suis&nbsp;: “J’utilise mon empreinte biométrique”. A noter que ce facteurs d'authentification seul, comme utiliser son empreinte, ou ses yeux par scan ou biométrique n'est pas fiable ni robuste
...
Il est atteint de cyberagnétose. Cette condition fait que lorsqu’on regarde un support numérique on peut ressentir un malaise, une envie de vomir. C’est un malaise créé par la contradiction entre le vestibule (une partie de l'oreille) et la vision. C’est le même phénomène que le mal des transports.
...
Ce problème est causé par exemple par un `<br>` qui coupe un texte. On peut utiliser un `role=presentation` sur le `<br>` pour le camoufler. Donc il faut faire attention aux balises car selon le lecteur d'écran elles peuvent être prises en compte différemment. Les lecteurs d'écran analysent même le CSS.
