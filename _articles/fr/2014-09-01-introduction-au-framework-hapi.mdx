...
Relançons notre serveur , rendons nous à l’adresse [http://localhost:300](http://localhost:300)0 … Notre 404 disparaît au profit d’un (sommaire, il est bien vrai !) ‘Hello World!’ .
...
2.  path : le chemin absolu vers la requête (attention, il doit commencer par un ‘/’). Le router d’Hapi nous permet d’extraire des segments de l’URL. Ainsi on pourra créer des routes de ce type
    -   ‘/hello’ : cette route répondra exclusivement à la  requête [http://localhost:3000/hello](http://localhost:3000/hello)
    -   ‘/hello/\{name\}’ : cette route répondra ainsi sur [http://localhost:3000/hello/jean](http://localhost:3000/hello/jean)  ou encore  [http://localhost:3000/hello/isabelle](http://localhost:3000/hello/isabelle) . On accède au paramètre ‘name’ en ayant recours à request.params.name.
...
Relançons notre navigateur , rendons nous à l’adresse : [http://localhost:3000/hello/john/doe?occupation=developper](http://localhost:3000/hello/john/doe?occupation=developper)
...
Si nous essayons des mauvais paramètres, par exemple [http://localhost:3000/hello/j/doe?occupation=developper](http://localhost:3000/hello/j/doe?occupation=developper) … badaboum ! Hapi nous renvoie automatiquement une 400 (Bad Request) car la longueur du paramètre \{firstName\} est de 1 caractère. Or, dans nos règles de validation, nous avons précisé "min(3).max(20)".
...
1.  *plugin*: l’interface plugin que nous utiliserons pour définir des routes, des méthodes. Je ne détaillerai pas ici l’ensemble des propriétés et des méthodes que nous offre ici le framework, je vous laisse un lien vers la documentation [https://github.com/hapijs/hapi/blob/master/docs/Reference.md#root-methods-and-properties](https://github.com/hapijs/hapi/blob/master/docs/Reference.md#root-methods-and-properties)
...
Ici, on a créé deux serveurs, chacun ayant un propre label, notion que l’on pourrait comparer à un *tag*. Ensuite, on a explicitement déclaré une route pour le serveur avec le label *admin*. On a ensuite démarré notre pack. On y a inscrit notre plugin-example, c’est pourquoi l’URL ‘/plugin-example’ est disponible à la fois à l’adresse [http://localhost:8000/plugin-example](http://localhost:8000/plugin-example) mais aussi sur [http://localhost:8001/plugin-example](http://localhost:8001/plugin-example) !
