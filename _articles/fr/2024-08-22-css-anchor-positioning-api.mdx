...
<Figure src="{BASE_URL}/imgs/articles/2024-08-22-css-anchor-positioning-api/basic-schema.jpg?width=400" alt="Schéma basique d'un bouton (ancre) et son tooltip (élément positionné)">*Schéma basique d'un bouton (ancre) et son tooltip (élément positionné)*</Figure>
...
<Figure src="{BASE_URL}/imgs/articles/2024-08-22-css-anchor-positioning-api/placed-anchor.jpg?width=600" alt={"Schéma qui présente le tooltip positionné en haut à droite par rapport au bouton. Quels côtés se touchent ? Le \"bottom left\" du tooltip et le \"top right\" de l'ancre"}>*Schéma qui présente le tooltip positionné en haut à droite par rapport au bouton*</Figure>
...
`justify` va s'appliquer sur le `main-axis` (ici l'axe horizontal) et `align` sur le `cross-axis` (ici l'axe vertical).

</Reminder>

<Reminder variant="warning" title="Attention">

D'après mes tests cela ne semble pour l'instant fonctionner qu'avec la syntaxe implicite, donc en ajoutant `position-anchor` et le nom de l'ancre.
...
A partir de Chromium 129 `inset-area` s'appellera `position-area`

</Reminder>

<Figure src="{BASE_URL}/imgs/articles/2024-08-22-css-anchor-positioning-api/9-cells-grid.jpg?width=500" alt="Schéma qui présente la grille à 9 cases : dans une grille de 3 lignes sur 3 colonnes, la case centrale contient le bouton et il y a 8 autres cases tout autour du bouton">*Schéma qui présente la grille à 9 cases*</Figure>
...
Encore une fois on va pouvoir utiliser les valeurs logiques (avec `block` et `inline`).
...
À partir de Chromium 128 `position-try-options` s'appellera `position-try-fallbacks`
...
`flip-block flip-inline` permet uniquement de passer au côté d'en face, il ne passe pas "sur le côté" lorsqu'il n'y a plus d'espace comme ce qu'on a fait juste avant avec l'enchainement de positions dans `position-try-options`.
