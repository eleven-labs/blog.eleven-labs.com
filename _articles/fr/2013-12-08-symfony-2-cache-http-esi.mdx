...
Le système de cache de Symfony vous permettra de diminuer les temps de réponses de vos pages en utilisant la puissance du cache HTTP tel qu'il est défini dans la [spécification HTTP](https://fr.wikipedia.org/wiki/Hypertext_Transfer_Protocol). Sachez cependant que ce n’est qu’une des solutions possibles.
...
On constate que le système de cache est bien activé. Pour aller plus loin avec les différentes options possibles de l’en-tête, je vous invite fortement, si ce n’est pas déjà fait, à lire la doc sur le site officiel de [Symfony](http://symfony.com/doc/master/http_cache.html).
