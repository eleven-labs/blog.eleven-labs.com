...
L'idée est donc de le faire à chaque commit. Pour cela rien de plus simple, on ajoute [un hook de pre-commit](https://git-scm.com/docs/githooks){:target="_blank" rel="nofollow"}.
...
Une fois cela validé, la suite logique est de faire en sorte que les développeurs codent tous avec les mêmes standards. Ce qui est bien, c'est que PHP a déjà des standards : les **[PSR](http://www.php-fig.org/psr/){:target="_blank" rel="nofollow"}**.

Encore faut-il que tous les développeurs les suivent, c'est assez simple en PHP. Nous avons ajouté dans notre hook de pre-commit la commande de vérification disponible dans cet article, *[vérifier la qualité du code](https://blog.eleven-labs.com/fr/verifier-la-qualite-du-code/){:target="_blank" rel="nofollow"}*
...
Nous avons donc regardé les solutions du marché, et comme nous utilisions Github, [Travis](https://travis-ci.org){:target="_blank" rel="nofollow"} était la plus approprié.
...
Nous avons choisi d'utiliser [l'editorconfig](http://editorconfig.org/){:target="_blank" rel="nofollow"} ! L'editorconfig est un fichier que l'on ajoute à la racine du repo et qui est utilisé par la plupart des IDE pour vérifier en live la syntaxe.
...
Passons au code ! Comme tout le monde, nous avions des tests unitaires et fonctionnels en [phpunit](https://phpunit.de/){:target="_blank" rel="nofollow"}.  Comme nous avions Travis, qui était en place, il fallait seulement ajouter le script pour les lancer dans la configuration.
...
L'idée fut d'avoir un juge de touche Nous avons cherché et nous avons choisi [Scrutinizer](https://scrutinizer-ci.com/). [Scrutinizer](https://scrutinizer-ci.com/){:target="_blank" rel="nofollow"} est une solution qui permet de *juger* votre code. Il vous donne une note en prenant en compte plusieurs signes de qualité :
...
La mise en place est simple puisque [Scrutinizer](https://scrutinizer-ci.com/){:target="_blank" rel="nofollow"} se plugue facilement à Github. Il suffit d'ajouter un fichier `.scrutinizer.yml` dans votre projet.
...
Encore une fois, nous avons commencé par la syntaxe avec la mise en place [Eslint](https://eslint.org/). Nous avons choisi comme standard la configuration de [airbnb](https://www.npmjs.com/package/eslint-config-airbnb){:target="_blank" rel="nofollow"}.
...
Nous avons alors fait les tests unitaires. Comme toujours le choix de la techno ne fut pas simple. Mais comme nous faisions du [react](https://facebook.github.io/react/), [jest](https://facebook.github.io/jest/){:target="_blank" rel="nofollow"} s'est très vite imposé.
...
Comme nous le savions de notre expérience en PHP, tout cela ne suffisait pas. Nous avons donc cherché un outil équivalent à Scrutinizer et nous avons trouvé [Bithound](https://www.bithound.io){:target="_blank" rel="nofollow"}. Cet outil est un peu moins poussé, mais il permet de mettre une note sur votre code et surtout de vous alerter quand des librairies extérieures ne sont plus à jour.
...
Jamais deux sans trois, on commence par la syntaxe. Pour cela, nous avons utilisé [Stylelint](https://stylelint.io/){:target="_blank" rel="nofollow"} qui permet de gérer la syntaxe de vos fichiers CSS. Stylelint permet de nombreuses vérifications:
...
La seconde façon de vérifier la qualité du CSS est de faire des tests de non régression visuelle. Pour cela [BackstopJs](https://github.com/garris/BackstopJS){:target="_blank" rel="nofollow"} est une solution complète, qui permet de tester deux versions de votre code html/css.
...
Nous avons choisi [Speedcurve](https://speedcurve.com/){:target="_blank" rel="nofollow"}, un outil de monitoring simple d'utilisation et surtout qui permet de suivre les concurrents.
