...
En tant que développeur, la sécurisation de nos applications web est devenu un enjeu majeur. Dans un monde où la data vaut de de l’or, il est capital de la conserver secrète. Let’s Encrypt, permettant de générer des certificat SSL gratuitement et Google, qui augmente le ranking des sites sécurisés, ont favorisé la mise en place progressive du protocole HTTPS sur une bonne partie des applications web. 
...
Depuis PHP 7.2, une nouvelle librairie cryptographique est directement intégré au coeur de PHP : libsodium. Elle permet de remplacer la très vieille extension mcrypt. Elle se base sur une librairie qui n'est plus maintenue depuis 2007. Il reste l'extension openssl mais elle ne supporte pas les derniers algorithme de chiffrement et de signature.
...
Le but de ce bundle est d'intégrer la librairie Sapient dans l'écosystème Symfony. Il permet de : 
 ...
 - utiliser des middlewares Guzzle et activable en une ligne de configuration

Il y a également des écouteurs Symfony pour déchiffrer et chiffrer les réponses directement. Dans le cas où vous avec des besoins avancés, il y a la librairie Sapient directement accessible en tant que service. 

Comme vous pouvez le voir, ce bundle s'appuie sur les composants revu par des experts en cryptographie. Et comme dis un dicton : "Don't roll your own crypto" (Ne faite pas votre propre crypto), sinon c'est la porte ouverte à tous les problèmes. 

## Chiffrer et signer les réponses de l'API

Nous avons une API d'une banque A et un client B qui souhaite consulter la quantité d'argent sur son compte. Le client B veux que les données de l'API soient chiffrées et signées, sinon elle les réfusent. L'équipe de l'API A utilise Symfony 4 et il va mettre en place ce chiffrement et la signature avec le bundle Sapient.
...
La recette va vous inviter à créer la paire de clef. Il suffit de lancer la commande suivante et de copier-coller le contenue dans _config/packages/sapient.yml_
...
La clef publique de client B est installé dans la partie _sealing_public_keys_ (clef publique de scellement). C'est cette clef qui va être utilisé pour chiffrer le contenu. Côté client B, la clef publique d'API A est installé dans la partie _verifying_public_keys_ (clef publique de vérification). Comme son nom l'indique, c'est la clef publique d'API A qui permet de vérifier que la réponse provient bien d'API A.
...
Il est possible de pousser ce cas d'usage plus loin avec le chiffrement et la signature des requêtes envoyé par le client B. Ainsi, seul l'API A est en mesure de comprendre les requêtes. Cet exemple est disponible dans la [documentation](https://sapient-bundle.readthedocs.io/en/latest/configuration.html#sign-and-seal-request). 

## Conclusion

Le bundle s'installe et se configure simplement. Il rend accessible un domaine jusque là réservé à des personnes expérimenté. L'introduction d'une librairie cryptographique dans le coeur de PHP rend le langage plus fort. Cela va permettre de sécuriser d'avantage les applications présentes et futures, et effacer petit à petit cette image de langage plein de trou de sécurité. 
...
L'une des forces de la librairie Sapient est de reposer sur un composant de sécurité intégré au coeur de PHP et d'avoir déjà été audité : libsodium. Ce composant expose les algorithme les plus robuste à ce jour : Ed25519, XChaCha20-Poly1305, X25519, BLAKE2b, HMAC-SHA512-256.
...
Il est possible de décomposer le processus de cette façon : 
 ...
 - elle passe dans la fonction de chiffrement
 - une paire de clef asymétrique est créé avec l'algorithme X25519
 - un nonce est également créé
 - un échange de clef est effectué pour obtenir une clef partagé avec l'algorithme X25519
 - cette clef passe dans une fonction de hash BLAKE2b pour assurer l'intégrité de la clef partagé
 - le corps de la réponse est chiffré avec cette clef partagé en utilisant l'algorithme XChaCha20-Poly1305
 - la clef publique éphémère est concaténé avec le corps de la réponse chiffré
 - le tout est de nouveau encapsulé dans un objet _Response_ PSR-7

Ensuite côté client B, le processus inverse est effectué :
 - un objet _Response_ PSR-7 est créé
 - elle passe dans la fonction de déchiffrement
 - la clef publique éphémère est détaché du corps de la réponse
 - un nonce est également créé
 - un échange de clef est effectué pour obtenir une clef partagé avec l'algorithme X25519
 - le corps de la réponse est déchiffré avec cette clef partagé en utilisant l'algorithme XChaCha20-Poly1305
 - le message déchiffré est de nouveau encapsulé dans un objet _Response_ PSR-7
   
Lors du déchiffrement, l'algorithme vérifie que le message n'a pas été altéré. Si c'est le cas, une exception _\SodiumException_ est levé avec le message _Invalid MAC_ (MAC = Message Authenticated Cipher).
