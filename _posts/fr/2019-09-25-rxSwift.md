...
---
Avant d’entrer dans le vif du sujet qu'est RxSwift, voyons d'abord ce qu'est la programmation réactive. 

## Programmation  réactive

La programmation réactive parle uniquement de flux et de données. 

Un flux crée des données à différents moments dans le temps. Un observateur sera lui notifié quand ces données seront récupérées, afin qu'il puisse réaliser une action spécifique avec celles-ci. 

## RxSwift

Tout d’abord, RxSwift est une librairie qui va permettre d’écrire un code asynchrone d’une manière plus concise et compacte. Il est basé sur les observables qui eux, sont une séquence de data à laquelle on peut réagir comme par exemple dans le cas où un utilisateur appuie sur un bouton. 
