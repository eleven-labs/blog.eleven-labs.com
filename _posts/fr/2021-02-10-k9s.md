...
Voila, vous savez désormais exécuter différentes commandes qui vous permettent de changer de vues. C'est déjà pas mal ! Vous êtes désormais capables de lister les différentes ressources de votre cluster et de voir si elles sont OK ! Allons un peu plus loin maintenant.
...
![Changement de namespace vue pod touche numérique]({{ site.baseurl }}/assets/2021-01-20-k9s/gif_numeric_parameter.gif){: .center-image .no-link-style}

Dans l'exemple ci-dessus, mon écran se vide lorsque je sélectionne le namespace *default* (`<2>`) car je n'ai pas de pod sur ce namespace. Je vous invite à jeter un oeil au titre du panneau principal si vous vous retrouvez sur un écran vide.
...
![Shell conteneur]({{ site.baseurl }}/assets/2021-01-20-k9s/gif_describe_next_match.gif){: .center-image .no-link-style}
...
## Actions groupées
On peut "marquer" des resources pour leur appliquer une action commune (e.g. supprimer 2 pods d'un coup). On marque la resource en surbrillance en appuyant sur `espace`. La resource change alors de couleur et on appuiera de nouveau sur `espace` pour retirer la marque.

![Shell conteneur]({{ site.baseurl }}/assets/2021-01-20-k9s/gif_basic_markers.gif){: .center-image .no-link-style}

On peut également marquer une plage de resource. Pour cela, on commencera par mettre en surbrillance la première resource de la plage ; la marquer avec `espace` ; mettre la dernière resource de la plage en surbrillance et presser `ctrl+espace` pour marquer toutes les resources comprises entre les 2. Il n'existe pas de moyen d'enlever la marque d'une plage entière.

![Shell conteneur]({{ site.baseurl }}/assets/2021-01-20-k9s/gif_marker_range.gif){: .center-image .no-link-style}

Notez bien que dès qu'on marque au moins une resource, l'action ne s'applique plus sur la ligne en surbrillance, mais sur les resources marquées ! Dans l'exemple ci-dessous ce sont toutes mes resources marquées qui sont *killed* via `ctrl+k` et non pas la ligne en surbrillance.

![Shell conteneur]({{ site.baseurl }}/assets/2021-01-20-k9s/gif_marker_delete.gif){: .center-image .no-link-style}
