...
Le terme suivant sera action, j'entends par action: "Fais telle chose" dans mon application, pour la durée de l'article, ce sera un output console, une ouverture de page web, ce genres de choses.<br/>
...
Notre but est de réaliser une application qui contient des features, mais qui n'a pas structure comportementale.<br/>
Chaque feature va être définit comme un module, donc du code bien séparé qui vie sa vie et qui est disponible sans aucune dépendance au moment où il doit être utilisé.<br/>
...
Notre objectif ici est de réaliser une application qui pourra charger à chaud des modules et de leur faire exécuter des actions qui leur sont propres, mais sans que la séquence de ces actions soit définit dans le code de notre application.
...
```
Ce module est très simple, il contient deux méthodes, **sayHello** et **sayGoodBye**, ces deux méthodes vont écrire sur la console le message définit à l'intérieur de celle-ci.
...
```
Ce troisième module contient une seule méthode, il va ouvrir une instance safari et aller sur l'url passé en paramètre, rien de bien compliqué.


Comme vous pouvez voir, les modules n'ont aucune intelligence, ils font juste ce qui leur est demandé.


#### L'application

Intéressons nous maintenant à l'application, c'est le coeur du projet.<br/>
En effet, comme définit plus haut, nous allons agir en 3 étapes<br/>
...
Là rien de bien méchant, on fait un GET avec la librairie Alamofire et on transforme ce JSON en GenericProtocol (une classe créer spécialement pour récupérer nos infos) via une petite classe perso que j'utilise toujours dans mes projets.
...
Notre GenericProtocol a un nom, une liste d'actions et un realObject. Je reviendrais plus tard sur ce realObject.<br/>
...
@end
```
Donc, si je résume, on a récupéré notre JSON via un call HTTP, transformé ce JSON en objet et instancier des classes qui contiennent des actions (nos modules).<br/>
...
En même temps appeler des méthodes sur des objets au Runtime, vous doutiez bien que ça n'allait pas être super clean et être fait en 2 lignes.<br/>
...
Si l'action a un paramètre alors on exécute cet action en lui passant le paramètre.<br/>
...
Bon, maintenant, c'est bien on a une application qui peut avoir une comportement dicté à distance, mais si on se penchait un peu plus sur ce comportement?<br/>
On enchaine?
...
On a définit le comportement de notre application.<br/>
