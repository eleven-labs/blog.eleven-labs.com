...
RabbitMQ is a message broker, allowing to process things asynchronously. There's already an [article](https://blog.eleven-labs.com/fr/creer-rpc-rabbitmq/){:target="_blank" rel="nofollow noopener noreferrer"} written about it, if you're not familiar with RabbitMQ.
...
*   [RabbitMQ Admin Toolkit](https://github.com/odolbeau/rabbit-mq-admin-toolkit){:target="_blank" rel="nofollow noopener noreferrer"} : PHP library the interacts with the HTTP API of our RabbitMQ server, to create exchanges, queues...
*   [Swarrot](https://github.com/swarrot/swarrot){:target="_blank" rel="nofollow noopener noreferrer"} : PHP library to consume and publish our messages.

Swarrot is compatible with the amqp extension of PHP, as well as the [php-amqplib](https://github.com/php-amqplib/php-amqplib) library. The PHP extension has a certain advantage on performance (written in C) over the library, based on [benchmarks](https://odolbeau.fr/blog/benchmark-php-amqp-lib-amqp-extension-swarrot.html). To install the extension, click [here](https://serverpilot.io/community/articles/how-to-install-the-php-amqp-extension.html){:target="_blank" rel="nofollow noopener noreferrer"}.
The main adversary to Swarrot, [RabbitMqBundle](https://github.com/php-amqplib/RabbitMqBundle){:target="_blank" rel="nofollow noopener noreferrer"}, is not compatible with the PHP extension, and is not as simple in both configuration and usage.

## Configuration

Our first step will be to create our RabbitMQ configuration: our exchange and our queue.

The RabbitMQ Admin Toolkit library, developed by _[odolbeau](https://github.com/odolbeau){:target="_blank" rel="nofollow noopener noreferrer"},_ allows us to configure our vhost very easily. Here is a basic configuration declaring an exchange and a queue, allowing us to send our mascot Wilson and his fellow friends to space:
...
The library helping us to consume and publish messages, Swarrot, has a Symfony bundle which will help us use it very easily in our app: [SwarrotBundle](https://github.com/swarrot/SwarrotBundle){:target="_blank" rel="nofollow noopener noreferrer"}.
...
I'm going to show you how to proceed, keeping our example _send_astronaut_to_space._ Let's decide that we're going to retry the publication of our message 3 times maximum. To do that, we need 3 retry queues. Fortunately, configuration of retry queues and exchanges is so easy with [RabbitMQ Admin Toolkit](https://github.com/odolbeau/rabbit-mq-admin-toolkit){:target="_blank" rel="nofollow noopener noreferrer"}: we only need one line! Let's see this more closely :
...
*   [RabbitMQ Admin Toolkit](https://github.com/odolbeau/rabbit-mq-admin-toolkit){:target="_blank" rel="nofollow noopener noreferrer"}
*   [Swarrot](https://github.com/swarrot/swarrot){:target="_blank" rel="nofollow noopener noreferrer"}
